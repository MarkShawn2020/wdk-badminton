...
export const memberRegistrationSchema = z.object({
  ...
  // 职业信息
  companyName: z.string().max(200, '公司/组织名称不能超过200个字符').optional().or(z.literal('')),
  companyNameEn: z.string().max(200, '公司英文名不能超过200个字符').optional().or(z.literal('')), // Keep for backward compatibility
  jobTitle: z.string().max(100, '职位不能超过100个字符').optional().or(z.literal('')),
  aiSector: z.string().max(150, 'AI领域不能超过150个字符').optional().or(z.literal('')),
  companyStage: z.enum(['SEED', 'SERIES_A', 'SERIES_B', 'SERIES_C', 'GROWTH', 'MATURE']).optional(), // Keep for backward compatibility
